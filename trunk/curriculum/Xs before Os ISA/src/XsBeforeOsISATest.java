...
public class XsBeforeOsISATest extends TestCase {
	...
	public void testIsOK1 ( ) {
		char [ ] [ ] tests = {{'X'}, {'O'}};
		for (int k=0; k<tests.length; k++) {
			assertTrue ("fails for one-element array", XsBeforeOsISA.isOK (tests[k], 0));			
		}
	}
	...
	public void testIsOK2 ( ) {
		...
		for (int k=0; k<tests.length; k++) {
			assertEquals ("fails for " + toString (tests[k]), XsBeforeOsISA.isOK (tests[k], 1), solns[k]);			
		}
	}
	...
	public void testIsOK3 ( ) {
		...
		for (int k=0; k<tests.length; k++) {
			assertEquals ("fails for " + toString (tests[k]), XsBeforeOsISA.isOK (tests[k], 2), k2solns[k]);
			assertEquals ("fails for " + toString (tests[k]), XsBeforeOsISA.isOK (tests[k], 3), k3solns[k]);
		}
	}
